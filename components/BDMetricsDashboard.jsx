...
const BDMetricsDashboard = () => {
  const data = [
    ...
    { month: "Sep '24", meetings: 13, event: "Reduced Hours" },
  ...
  const calculateMetrics = () => {
    ...
    const afterAvg = afterJoining.length > 0 ? afterTotal / afterJoining.length : 0;

    // Calculate peak performance metrics
    const peakValue = Math.max(...data.map(item => item.meetings));
    const annualizedPeak = peakValue * 12;

    // Calculate growth streak (Not currently used in the UI but calculated for potential future use)
    let maxStreak = 0;
    let currentStreak = 0;
    ...
    // Calculate recovery rate after reduced hours
    const reducedHoursStartIndex = data.findIndex(item => item.month === "Aug '24");
    const reducedHoursEndIndex = data.findIndex(item => item.month === "Sep '24");
    const recoveryMonthIndex = data.findIndex(item => item.month === "Oct '24");

    const reducedHoursAvg = (data[reducedHoursStartIndex].meetings + data[reducedHoursEndIndex].meetings) / 2
    const recoveryRate = (((data[recoveryMonthIndex].meetings - reducedHoursAvg) / reducedHoursAvg) * 100).toFixed(0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card className="bg-white/50 backdrop-blur-sm">
          ...
          <CardContent>
            ...
            <p className="text-sm text-blue-600">vs. Reduced Hours Average</p>
          </CardContent>
        </Card>
      </div>
